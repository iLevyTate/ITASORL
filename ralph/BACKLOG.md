...
P1 | fullruns (local machine) | RUN THE CG RE-SCORE: `scripts/reanalyze_cg_states.py` on the l3_h8_heldout and h7_reverse dumps; adjudicate with the frozen two-clause rule (tail >= 0.65 AND > untrained + 0.05, latetail is the decay diagnostic); resolve the PROVISIONAL banners in FINDINGS/README/CITATION/index.html in the same change (the recheck gate's qualifier guard will flag them).
P2 | run_expB2 dumps (GPU) | Behavior audit position gap: regenerate L3 h8 pools with `--dump-states` (dumps now carry pos_x/pos_y/heading trace channels) and re-run `scripts/audit_behavior_mediation.py`; until then the 0.726 behavior-independent number carries the FINDINGS 10.4 covariate-gap note.
